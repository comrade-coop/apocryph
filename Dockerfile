...
# BACKEND_ETH_RPC is the RPC used for connecting to Ethereum (should optimally be a wss:// link) (can also be passed as env)
ARG BACKEND_ETH_RPC=https://mainnet.base.org
# BACKEND_ETH_CHAIN_ID is the chain ID of the Ethereum RPC (can also be passed as env)
ARG BACKEND_ETH_CHAIN_ID=8453
# BACKEND_REPLICATE_SITES is a comma-separated list of VITE_GLOBAL_HOST-s that the backend should configure replication to (can also be passed as env)
ARG BACKEND_REPLICATE_SITES=""
...
ARG BACKEND_ETH_WITHDRAW
ARG BACKEND_ETH_RPC
ARG BACKEND_ETH_CHAIN_ID
ARG BACKEND_REPLICATE_SITES
ARG SIWE_DOMAIN
ARG AAPP_VERSION
...
ENV BACKEND_ETH_WITHDRAW=$BACKEND_ETH_WITHDRAW
ENV BACKEND_ETH_TOKEN=$VITE_TOKEN
ENV BACKEND_ETH_RPC=$BACKEND_ETH_RPC
ENV BACKEND_ETH_CHAIN_ID=$BACKEND_ETH_CHAIN_ID
ENV BACKEND_EXTERNAL_URL=$VITE_GLOBAL_HOST
ENV BACKEND_REPLICATE_SITES=$BACKEND_REPLICATE_SITES
