...
contract Payment{
	...
	function createChannel(address provider, address token, uint256 amount, uint256 deadline, uint256 minAdvanceDuration, uint256 price)
	public
	notZero(amount)
	allowedTransfer(token,msg.sender,amount)
	{
	...
	function lockFunds(address provider, address token, uint256 amount) public notZero(amount) notExpired(msg.sender,provider, token){
		Channel memory funds = channels[msg.sender][provider][token];
	...
	function widthdraw(address token, address client) public{
	...
	function uploadMetrics(address client, address token) public{
		Channel memory funds = channels[client][msg.sender][token];
		// multiply units of execution (derived from the metric msg) with the price per execution
		uint256 amount = 5 * funds.price;
