...
contract PaymentTest is Test {
    ...
    function test_createChannel() public {
        ...
        vm.expectRevert();
        payment.createChannel(provider, podId, token, 1, 500);
    ...
    function test_deposit() public {
        ...
        token.approve(address(payment), 500);
        payment.deposit(provider, podId, token, 500);
    ...
    function test_withdraw() public {
        ...
        vm.expectRevert(Payment.AmountRequired.selector);
        payment.withdrawUpTo(publisher, podId, token, 50, address(0));
    ...
    function test_unlock() public {
        ...
        payment.unlock(provider, podId, token);

        payment.createChannel(provider, podId, token, 20, 500);

        vm.expectRevert(Payment.ChannelLocked.selector);
        payment.withdrawUnlocked(provider, podId, token);

        vm.startPrank(provider);
        payment.withdraw(publisher, podId, token, 100, address(0));
        ...
        vm.expectRevert(Payment.AlreadyExists.selector);
        payment.createChannel(provider, podId, token, 20, 400);

        payment.closeChannel(provider, podId, token);
    ...
    function test_unlock_withdraw() public {
        ...
        vm.startPrank(provider);
        payment.withdrawUpTo(publisher, podId, token, 200, address(0));
        assertEq(token.balanceOf(provider), 200);
