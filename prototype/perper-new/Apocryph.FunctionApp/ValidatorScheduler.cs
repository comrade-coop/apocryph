...
namespace Apocryph.FunctionApp
{
    public static class ValidatorScheduler
    {
        ...
        public static async Task Run([PerperStream("ValidatorScheduler")] IPerperStreamContext context,
            [PerperStream("validatorSet")] IAsyncEnumerable<Dictionary<string, ValidatorSet>> validatorSetsStream,
            ...
            await validatorSetsStream.ForEachAsync(async validatorSets =>
            {
                ...
                foreach (var kv in validatorSets)
                {
                    if (!runningStreams.ContainsKey(kv))
                    {
                        ...
                        runningStreams[kv] = await context.CallStreamAction("ValidatorLauncher", new
                        {
                            ipfsGateway,
                            agentId = agentId,
