  ...
  if (isField(value)) {
    ...
    for (let key in value) {
      if (value.hasOwnProperty(key)) {
        let inner = toElement(value[key], {keyToText})
        let row = document.createElement((inner instanceof HTMLInputElement) ? 'label' : 'div')
...
export function toJSON<T>(value: FieldOrRaw<T>): T {
  if (isField(value)) {
    ...
    value = value as RecordField<T>  // (Huh?)
    let result: {[P in keyof T]?: T[P]} = Array.isArray(value) ? [] as any : {} /// Siigh
    for (let key in value) {
      if (value.hasOwnProperty(key)) {
...
export type EncodingOptions = {encoding: keyof typeof Encodings}
export type SpinnerOptions<T> = {min?: T, max?: T, step?: T}
...
export function field(initialValue: string, options?: {}): Field<string>
